...
public class Calculus {
     ...
     * @param mPixelSpacingX - information about pixel spacing on imgae from DICOM file,
     *                       first part of DICOM tag 0028:0030
     * @param mPixelSpacingY - information about pixel spacing on image from DICOM file,
     *                       second part of DICOM tag 0028:0030
     * @param mScaleFactor   - current scale factor of image on which try to calculate the distance.
     * @return - distance in mm.
     */
    public static float getRealDistance(PointF[] mPoints,
                                        float mPixelSpacingX, float mPixelSpacingY,
                                        double mScaleFactor) {
        float distanceX = Math.abs(mPoints[0].x - mPoints[1].x) * mPixelSpacingX;
        float distanceY = Math.abs(mPoints[0].y - mPoints[1].y) * mPixelSpacingY;
